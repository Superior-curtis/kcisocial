...
import { Users, Info, Grid3X3, ArrowLeft, Heart, MessageCircle, Image as ImageIcon, MessageSquare, Send, Paperclip, Plus, Trash2, Crown, X, UserPlus, Play, Search, ExternalLink, Music } from 'lucide-react';
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="border-t border-border">
          ...
          <TabsContent value="music" className="mt-0 p-4">
            <div className="space-y-4">
              {/* Search Form */}
              <form onSubmit={handleSearchMusic} className="space-y-3 bg-gradient-to-r from-orange-500/10 to-red-500/10 p-4 rounded-lg border border-orange-500/20">
                <div>
                  <label className="text-sm font-medium flex items-center gap-2">
                    🎵 Share Music in the Club
                    <span className="text-xs text-muted-foreground font-normal">(Search & Share)</span>
                  </label>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {musicSearchResults.map((track) => (
                      <div key={track.id} className="flex items-center gap-3 p-3 rounded-lg bg-accent hover:bg-accent/80 transition">
                        {track.image ? (
                        ...
                        <div className="flex gap-1 flex-shrink-0">
                          ...
                          <a
                            ...
                            className="inline-flex items-center gap-1 px-3 py-2 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded text-xs font-medium transition"
                            title="Play on KKBOX"
                          >
              ...
              {/* Now Playing / Shared Tracks */}
              {sharedTracks.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="font-semibold text-sm text-muted-foreground uppercase">Club Playlist ({sharedTracks.length})</h3>
                  </div>
                  <div className="space-y-2">
                    {sharedTracks.map((track, idx) => (
                      <div key={track.id} className="flex items-center gap-3 p-3 bg-gradient-to-r from-orange-500/5 to-red-500/5 rounded-lg border border-orange-500/20 hover:border-orange-500/40 transition">
                        ...
                        <div className="flex gap-1 flex-shrink-0">
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            onClick={() => setSharedTracks(sharedTracks.filter(t => t.id !== track.id))}
                  ...
                  <div className="p-3 bg-orange-500/10 border border-orange-500/20 rounded-lg text-xs text-orange-700">
                    🎵 <strong>Music Sharing Room:</strong> Share your favorite tracks with club members! Click KKBOX or preview button to listen.
                  </div>
                </div>
              )}
